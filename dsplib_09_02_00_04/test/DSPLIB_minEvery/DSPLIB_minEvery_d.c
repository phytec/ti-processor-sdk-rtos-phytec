...
#include "DSPLIB_minEvery_idat.h"
...
int DSPLIB_minEvery_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   int32_t              tpi;
   ...
   minEvery_testParams_t *prm;
   minEvery_testParams_t  currPrm;
   minEvery_getTestParams(&prm, &test_cases);

   DSPLIB_minEvery_InitArgs kerInitArgs;

#if DSPLIB_DEBUGPRINT
   printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 0\n");
#endif

   int32_t             handleSize = DSPLIB_minEvery_getHandleSize(&kerInitArgs);
   DSPLIB_kernelHandle handle     = malloc(handleSize);

#if DSPLIB_DEBUGPRINT
   printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 1\n");
#endif

   TI_profile_init("DSPLIB_minEvery");

   #if (!(EVM_TEST))
      // file IO for Loki benchmarking
      FILE *fpOutputCSV = fopen("DSPLIB_minEvery.csv", "w+");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
...
#if DSPLIB_DEBUGPRINT
         printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d data_type %d dataSize %d \n", bufParamsIn.data_type,
                kerInitArgs.dataSize);
         printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d pIn1  %p pIn2 %p\n", pIn1, pIn2);
#endif
        ...
        status_init = DSPLIB_minEvery_init_checkParams(handle, &bufParamsIn,
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 1 status_init %d\n", status_init);
#endif
            if (status_init == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               DSPLIB_asm(" MARK 0");
               kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
               status_init           = DSPLIB_minEvery_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
               DSPLIB_asm(" MARK 1");
               TI_profile_stop();
            }
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 2 status_init %d\n", status_init);
#endif
            status_opt = DSPLIB_minEvery_exec_checkParams(handle, pIn1, pIn2, pOut);
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 3 status_opt %d\n", status_opt);
#endif
            if (status_opt == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               DSPLIB_asm(" MARK 2");
               status_opt = DSPLIB_minEvery_exec(handle, pIn1, pIn2, pOut);
               DSPLIB_asm(" MARK 3");
               TI_profile_stop();
            }
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 4 status_opt %d\n", status_opt);
#endif
            // initialize the kernel to use the natural C variant
            DSPLIB_minEvery_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            DSPLIB_asm(" MARK 8");
            status_nat = DSPLIB_minEvery_exec(handle, pIn1, pIn2, pOutCn);
            DSPLIB_asm(" MARK 9");
            TI_profile_stop();
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 6 status_nat %d\n", status_nat);
#endif
            if (currPrm.dataType == DSPLIB_FLOAT32 || currPrm.dataType == DSPLIB_FLOAT64) {
            ...
            comparisonDone = 1;
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 7 comparisonDone %d status_nat_vs_opt %d\n", comparisonDone,
                   status_nat_vs_opt);
#endif
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 8 status_nat_vs_opt %d status_ref_vs_opt %d "
                   "currentTestFail %d\n",
                   status_nat_vs_opt, status_ref_vs_opt, currentTestFail);
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 8 status_init %d status_opt %d status_nat %d\n",
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minEvery_d CP 8 fail %d\n", fail);
#endif
            ...
            #if (!(EVM_TEST))
               // write to CSV, must happen prior to write to screen because TI_profile_formina_min clears values in
               // counters
                fprintf(fpOutputCSV, "minEvery, %d, %d, %d, Data size:%d, %d, %d, %d, %lu, %lu, %.2f, %d\n", testNum,
...
#endif
   {
      return DSPLIB_minEvery_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
  ...
  status_init = DSPLIB_minEvery_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
  ...
  status_init = DSPLIB_minEvery_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
  ...
  status_init = DSPLIB_minEvery_exec_checkParams(handle, NULL, pIn2, pOut);  
  currentTestFail = (status_init != DSPLIB_ERR_NULL_POINTER);
  fail = ((fail == 1) || currentTestFail == 1) ? 1 : 0;
  sprintf(desc, "%s", "COVERAGE TEST");
  TI_profile_add_test(testNum, 0,0,0,currentTestFail,desc);
  ...
  status_init = DSPLIB_minEvery_exec_checkParams(handle, pIn1, NULL, pOut);  
  currentTestFail = (status_init != DSPLIB_ERR_NULL_POINTER);
  fail = ((fail == 1) || currentTestFail == 1) ? 1 : 0;
  sprintf(desc, "%s", "COVERAGE TEST");
  TI_profile_add_test(testNum, 0,0,0,currentTestFail,desc);
  ...
  status_init = DSPLIB_minEvery_exec_checkParams(handle, pIn1, pIn2, NULL);  
  currentTestFail = (status_init != DSPLIB_ERR_NULL_POINTER);
  fail = ((fail == 1) || currentTestFail == 1) ? 1 : 0;
  sprintf(desc, "%s", "COVERAGE TEST");
  TI_profile_add_test(testNum, 0,0,0,currentTestFail,desc);
...
/* Main call for inminidual test projects */
...
/* ======================================================================== */
/*  End of file:  DSPLIB_minEvery_d.c                                       */
/* ======================================================================== */