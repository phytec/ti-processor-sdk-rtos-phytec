...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    idatFile.write("%s, // N \n" % ((N)))
    idatFile.write("%s, // K \n" % ((K)))
    idatFile.write("%s * sizeof(%s), // stride of A matrix \n" % ((strideIn0), (dType)))
    idatFile.write("%s * sizeof(%s), // stride of B matrix \n" % ((strideIn1), (dType)))
    idatFile.write("%s * sizeof(%s), // stride of C matrix \n" % ((strideOut), (dType)))
...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "DSPLIB_matMul_idat.h"'
    ...
    idatFile.write("static DSPLIB_matMul_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    if dType[0] == "i" or dType[0] == "u":
        in0 = np.random.randint(low=minVal, high=maxVal, size=(M, K)).astype("int")
        in1 = np.random.randint(low=minVal, high=maxVal, size=(K, N)).astype("int")
        out = DSPLIB_utils.shiftAndRound(np.matmul(in0, in1), shift)
    elif dType[0] == "f":
        in0 = np.random.uniform(low=minVal, high=maxVal, size=(M, K)).astype("float32")
        in1 = np.random.uniform(low=minVal, high=maxVal, size=(K, N)).astype("float32")
        out = np.matmul(in0, in1)
    elif dType[0] == "d":
        in0 = np.random.uniform(low=minVal, high=maxVal, size=(M, K)).astype("double")
        in1 = np.random.uniform(low=minVal, high=maxVal, size=(K, N)).astype("double")
        out = np.matmul(in0, in1)
...
idatFileName = "../../DSPLIB_matMul/DSPLIB_matMul_idat.c"
