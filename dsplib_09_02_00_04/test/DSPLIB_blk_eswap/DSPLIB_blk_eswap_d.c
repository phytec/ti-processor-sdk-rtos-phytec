...
#include "DSPLIB_blk_eswap_idat.h"
...
int DSPLIB_blk_eswap_d(uint32_t *pProfile, uint8_t LevelOfFeedback) {
  ...
  blk_eswap_testParams_t *prm;
  blk_eswap_testParams_t currPrm;
  blk_eswap_getTestParams(&prm, &test_cases);
  ...
  TI_profile_init("DSPLIB_blk_eswap");
#if !defined(EVM_TEST)
  // file IO for Loki benchmarking
  FILE *fpOutputCSV = fopen("DSPLIB_blk_eswap.csv", "w+");
  ...
  for (tpi = 0; tpi < test_cases; tpi++) {
    ...
    for (repCount = 0; repCount < numReps; repCount++) {
      ...
      if (pIn && pOut && pOutCn) {
        ...
        status_init = DSPLIB_blk_eswap_init_checkParams(handle, &bufParamsIn,
        ...
        if (status_init == DSPLIB_SUCCESS) {
          ...
          status_init = DSPLIB_blk_eswap_init(handle, &bufParamsIn, &bufParamsOut,
                                           &kerInitArgs);
          DSPLIB_asm(" MARK 1");
          TI_profile_stop();
        }

        status_opt = DSPLIB_blk_eswap_exec_checkParams(handle, pIn, pOut);

        if (status_opt == DSPLIB_SUCCESS) {
		  /* In place blk_eswap test cases */
          if((testNum >= 136) &&  (testNum <= 143)){
            DSPLIB_DEBUGPRINTFN(0, "%s", "Inplace swapping test case\n");
            copyRoi(pOut, pIn, bufParamsIn.dim_x, 1, 0, DSPLIB_sizeof(currPrm.dataTypeIn));
            TI_profile_start(TI_PROFILE_KERNEL_OPT);
            DSPLIB_asm(" MARK 2");
        
            status_opt = DSPLIB_blk_eswap_exec(handle, pOut, NULL);
            ...
            status_opt = DSPLIB_blk_eswap_exec(handle, pIn, pOut);
        ...
        status_init = DSPLIB_blk_eswap_init_checkParams(handle,
                                                       &bufParamsIn,
                                                       &bufParamsOut,
                                                       &kerInitArgs);
        if (status_init == DSPLIB_SUCCESS)
        {
          ...
          DSPLIB_asm(" MARK 8");
          status_init = DSPLIB_blk_eswap_init(handle,
                                             &bufParamsIn,
                                             &bufParamsOut,
                                             &kerInitArgs);
          DSPLIB_asm(" MARK 9");
        ...
        if (status_init == DSPLIB_SUCCESS) {
		  /* In place blk_eswap test cases */
          if((testNum >= 136) &&  (testNum <= 143)){
            copyRoi(pOutCn, pIn, bufParamsIn.dim_x, 1, 0, DSPLIB_sizeof(currPrm.dataTypeIn));
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            DSPLIB_asm(" MARK 10");
        
            status_opt = DSPLIB_blk_eswap_exec(handle, pOutCn, NULL);
            ...
            status_opt = DSPLIB_blk_eswap_exec(handle, pIn, pOutCn);
        ...
        if (currPrm.dataTypeIn == DSPLIB_FLOAT32 ||
          ...
          status_nat_vs_opt = TI_compare_mem_2D_float(
              (void *)pOut, (void *)pOutCn, 0.001, (double)powf(2, -10),
              bufParamsOut.dim_x, 1, 0, DSPLIB_sizeof(currPrm.dataTypeOut));
        } else {
          status_nat_vs_opt = TI_compare_mem_2D(
              (void *)pOut, (void *)pOutCn, 0, 0, bufParamsOut.dim_x, 1, 0,
        ...
        if (currPrm.staticOut != NULL) {
          status_ref_vs_opt = TI_compare_mem_2D(
              (void *)currPrm.staticOut, (void *)pOut, 2, 65535,
...
#if !defined(EVM_TEST)
        // write to CSV, must happen prior to write to screen because TI_profile_forsqra_sqr clears values in counters
          ...
          fprintf(fpOutputCSV, "blk_eswap, %d, %d, %d, -, %d, 1, 1, %d, %d, %3.2f\n",
        ...
        TI_profile_add_test(testNum++, currPrm.dataSize, hwCycles_local, 0, currentTestFail,
                            desc);
      } else {
...
int coverage_test_main()
{
   ...
   while (testNum <= 1005) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_blk_eswap_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_blk_eswap_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1001:
         ...
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_blk_eswap_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_blk_eswap_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1002:
         ...
         break;
      case 1003:
         status_nat      = DSPLIB_blk_eswap_exec_checkParams(handle, NULL, pOut);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1004:
         status_nat      = DSPLIB_blk_eswap_exec_checkParams(handle, pIn, NULL);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1005:
         bufParamsIn.data_type  = DSPLIB_INT128;
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_blk_eswap_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_blk_eswap_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         currentTestFail        = ((status_nat != DSPLIB_ERR_INVALID_TYPE) || (status_opt != DSPLIB_ERR_INVALID_TYPE));
         break;
      default:
         break;
...
int test_main(uint32_t *pProfile) {
...
#endif
  {
    ...
    return DSPLIB_blk_eswap_d(&pProfile[0], 0);
  }
}

/* Main call for inblk_eswapidual test projects */
...
/* ======================================================================== */
/*  End of file:  DSPLIB_blk_eswap_d.c                          */
/* ======================================================================== */
