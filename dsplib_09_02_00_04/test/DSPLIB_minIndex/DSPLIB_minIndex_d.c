...
#include "DSPLIB_minIndex_idat.h"
...
int DSPLIB_minIndex_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   int32_t              tpi;
   ...
   minIndex_testParams_t *prm;
   minIndex_testParams_t  currPrm;
   minIndex_getTestParams(&prm, &test_cases);

   DSPLIB_minIndex_InitArgs kerInitArgs;

#if DSPLIB_DEBUGPRINT
   printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 0\n");
#endif

   int32_t             handleSize = DSPLIB_minIndex_getHandleSize(&kerInitArgs);
   DSPLIB_kernelHandle handle     = malloc(handleSize);

#if DSPLIB_DEBUGPRINT
   printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 1\n");
#endif

   TI_profile_init("DSPLIB_minIndex");
   // file IO for Loki benchmarking
   /* FILE *fpOutputCSV = fopen("DSPLIB_minIndex.csv", "w+"); */
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
...
#if DSPLIB_DEBUGPRINT
         printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d data_type %d dataSize %d \n", bufParamsIn.data_type,
                kerInitArgs.dataSize);
         printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d pIn1  %p\n", pIn);
#endif
         ...
         if (pIn && pOut && pOutCn) {
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 1 status_init %d\n", status_init);
#endif
            kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
            status_init           = DSPLIB_minIndex_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
            if (status_init == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               DSPLIB_asm(" MARK 0");
               status_init           = DSPLIB_minIndex_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
               DSPLIB_asm(" MARK 1");
               TI_profile_stop();
            }
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 2 status_init %d\n", status_init);
#endif
            status_opt = DSPLIB_minIndex_exec_checkParams(handle, pIn, pOut);
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 3 status_opt %d\n", status_opt);
#endif
            if (status_opt == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               DSPLIB_asm(" MARK 2");
               // printf("\nopt exec\n");
               status_opt = DSPLIB_minIndex_exec(handle, pIn, pOut);
               DSPLIB_asm(" MARK 3");
               TI_profile_stop();
            }
            ...
            DSPLIB_minIndex_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 4 status_opt %d\n", status_opt);
#endif
            // initialize the kernel to use the natural C variant
            DSPLIB_minIndex_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            DSPLIB_asm(" MARK 8");
            // printf("\nnatc exec\n");
            status_nat = DSPLIB_minIndex_exec(handle, pIn, pOutCn);
            DSPLIB_asm(" MARK 9");
            TI_profile_stop();
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 6 status_nat %d\n", status_nat);
#endif
            ...
            comparisonDone = 1;
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 7 comparisonDone %d status_nat_vs_opt %d\n", comparisonDone,
                   status_nat_vs_opt);
#endif
            ...
            if (currPrm.staticOut != NULL) {
               ...
               status_ref_vs_opt = TI_compare_mem_2D(
                   (void *) currPrm.staticOut, (void *) pOut, 0, 65535, bufParamsOut.dim_x, 1,
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 8 status_nat_vs_opt %d status_ref_vs_opt %d "
                   "currentTestFail %d\n",
                   status_nat_vs_opt, status_ref_vs_opt, currentTestFail);
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 8 status_init %d status_opt %d status_nat %d\n",
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_minIndex_d CP 8 fail %d\n", fail);
#endif
            ...
            // write to CSV, must happen prior to write to screen because TI_profile_formula_min clears values in
            // counters
            /* fprintf(fpOutputCSV, "minIndex, %d, %d, %d, Data size:%d, %d, %d, %d, %lu, %lu, %.2f, %d\n", testNum, */
            /*         currPrm.testPattern, DSPLIB_sizeof(bufParamsIn.data_type) * 8, currPrm.dataSize, */
            /*         currPrm.dataSize, archCycles, cycles[TI_PROFILE_KERNEL_OPT], */
            ...
            // clear the counters between runs; normally handled by TI_profile_minIndex_test
...
#endif
   {
      return DSPLIB_minIndex_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   bufParamsOut.dim_x                  = 1;
   void                    *pIn        = (void *) malloc(bufParamsIn.dim_x * 8);
   void                    *pOut       = (void *) malloc(bufParamsOut.dim_x * 8);
   ...
   while (testNum <= 1003) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_minIndex_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_minIndex_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1001:
         ...
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_minIndex_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_nat             = DSPLIB_minIndex_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_nat             = DSPLIB_minIndex_exec_checkParams(handle, NULL, pOut);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_minIndex_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_opt             = DSPLIB_minIndex_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_opt             = DSPLIB_minIndex_exec_checkParams(handle, pIn, NULL);
         currentTestFail        = ((status_nat != DSPLIB_ERR_NULL_POINTER) || (status_opt != DSPLIB_ERR_NULL_POINTER));
         break;
      case 1002:
...
/* ======================================================================== */
/*  End of file:  DSPLIB_minIndex_d.c                                            */
/* ======================================================================== */
