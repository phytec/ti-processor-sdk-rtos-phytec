...
int DSPLIB_w_vec_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   int32_t              tpi;
   ...
   DSPLIB_w_vec_testParams_t *prm;
   DSPLIB_w_vec_testParams_t  currPrm;
   DSPLIB_w_vec_getTestParams(&prm, &test_cases);

   DSPLIB_w_vec_InitArgs kerInitArgs;

#if DSPLIB_DEBUGPRINT
   printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 0\n");
#endif
...
#if DSPLIB_DEBUGPRINT
   printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 1\n");
#endif

   TI_profile_init("DSPLIB_w_vec");

   // file IO for Loki benchmarking
   FILE *fpOutputCSV = fopen("DSPLIB_w_vec.csv", "w+");
   fprintf(fpOutputCSV, "Kernel, Test ID, Test type, Bit Width, Data Size, WeightStyle, Loki Cycle\n");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         // get weight input style (scalar vs vector)
         kerInitArgs.weightStyle = currPrm.weightFlag;
         ...
         // allocate space for weight input scalar/vector
         void *pM;
         // case: scalar weight input
         if (kerInitArgs.weightStyle == DSPLIB_SCALAR) {
            pM = (void *) TI_memalign(DSPLIB_L2DATA_ALIGNMENT, dataWidth);
         }
         // case: vector weight input
         else {
            pM = (void *) TI_memalign(DSPLIB_L2DATA_ALIGNMENT, bufParamsIn.dim_x * dataWidth);
         }
...
#if DSPLIB_DEBUGPRINT
         printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d data_type %d dataSize %d \n", bufParamsIn.data_type,
                kerInitArgs.dataSize);
         printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d pIn1  %p pIn2 %p\n", pIn1, pIn2);
#endif
         /* Only run the test if the buffer allocations fit in the heap */
         if (pIn1 && pIn2 && pM && pOut && pOutCn) {
            ...
            if (currPrm.dataType == DSPLIB_FLOAT32 || currPrm.dataType == DSPLIB_FLOAT64) {
               ...
               // fill weight buffer
               if (kerInitArgs.weightStyle == DSPLIB_SCALAR) {
                  TI_fillBuffer_float(prm[tpi].testPattern, 0, pM, prm[tpi].staticM, 1, 1, 0,
                                      DSPLIB_sizeof(currPrm.dataType), testPatternString);
               }
               // case: vector weight input
               else {
                  TI_fillBuffer_float(prm[tpi].testPattern, 0, pM, prm[tpi].staticM, bufParamsIn.dim_x, 1, 0,
                                      DSPLIB_sizeof(currPrm.dataType), testPatternString);
               }
            }
               ...
               // fill weight buffer
               if (kerInitArgs.weightStyle == DSPLIB_SCALAR) {
                  TI_fillBuffer(prm[tpi].testPattern, (uint8_t) 255, pM, prm[tpi].staticM, 1, 1, 0,
                                DSPLIB_sizeof(currPrm.dataType), testPatternString);
               }
               // case: vector weight input
               else {
                  TI_fillBuffer(prm[tpi].testPattern, (uint8_t) 255, pM, prm[tpi].staticM, bufParamsIn.dim_x, 1, 0,
                                DSPLIB_sizeof(currPrm.dataType), testPatternString);
               }
            }
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 1 status_init %d\n", status_init);
#endif
            kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
            status_init           = DSPLIB_w_vec_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 2 status_init %d\n", status_init);
#endif

            status_opt = DSPLIB_w_vec_exec_checkParams(handle, pIn1, pIn2, pM, pOut);

#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 3 status_opt %d\n", status_opt);
#endif
            if (status_opt == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               DSPLIB_asm(" MARK 2");
               status_opt = DSPLIB_w_vec_exec(handle, pIn1, pIn2, pM, pOut);
               DSPLIB_asm(" MARK 3");
               TI_profile_stop();
            }
            ...
            status_init           = DSPLIB_w_vec_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 4 status_opt %d\n", status_opt);
#endif
            // initialize the kernel to use the natural C variant
            DSPLIB_w_vec_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            DSPLIB_asm(" MARK 8");
            status_nat = DSPLIB_w_vec_exec(handle, pIn1, pIn2, pM, pOutCn);
            DSPLIB_asm(" MARK 9");
            TI_profile_stop();

#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 6 status_nat %d\n", status_nat);
#endif
            if (currPrm.dataType == DSPLIB_FLOAT32 || currPrm.dataType == DSPLIB_FLOAT64) {
               status_nat_vs_opt = TI_compare_mem_2D_float((void *) pOutCn, (void *) pOut, 0.001, (double) powf(2, -10),
                                                           bufParamsOut.dim_x, 1, 0, DSPLIB_sizeof(currPrm.dataType));
            }
            else {
            ...
            comparisonDone = 1;
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 7 comparisonDone %d status_nat_vs_opt %d\n", comparisonDone,
                   status_nat_vs_opt);
#endif
...
#if DSPLIB_DEBUGPRINT
            printf(
                "DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 8 status_nat_vs_opt %d status_ref_vs_opt %d currentTestFail %d\n",
                status_nat_vs_opt, status_ref_vs_opt, currentTestFail);
            printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 8 status_init %d status_opt %d status_nat %d\n", status_init,
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_w_vec_d CP 8 fail %d\n", fail);
#endif
            ...
            // write to CSV, must happen prior to write to screen because TI_profile_formula_w_vec clears values in
            // counters
            fprintf(fpOutputCSV, "w_vec, %d, %d, %d, %d, %d, %lu\n", testNum, currPrm.testPattern,
                    DSPLIB_sizeof(bufParamsIn.data_type) * 8, currPrm.dataSize, currPrm.weightFlag,
            ...
            // clear the counters between runs; normally handled by TI_profile_w_vec_test
...
#endif
   {
      return DSPLIB_w_vec_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   DSPLIB_kernelHandle   handle     = malloc(handleSize);
   int                   fail       = 0;
   void                 *pIn1       = (void *) malloc(16 * 4);
   void                 *pIn2       = (void *) malloc(16 * 4);
   void                 *pM         = (void *) malloc(16 * 4);
   void                 *pOut       = (void *) malloc(16 * 4);
   ...
   while (testNum <= 1007) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_w_vec_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_w_vec_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1001:
         ...
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_w_vec_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         bufParamsIn.data_type  = DSPLIB_INT128;
         bufParamsOut.data_type = DSPLIB_INT32;
         status_opt             = DSPLIB_w_vec_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1002:
         ...
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_w_vec_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_w_vec_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1004:
         ...
         status_nat             = DSPLIB_w_vec_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_nat             = DSPLIB_w_vec_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_nat             = DSPLIB_w_vec_exec_checkParams(handle, NULL, pIn2, pM, pOut);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_w_vec_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_opt             = DSPLIB_w_vec_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_opt             = DSPLIB_w_vec_exec_checkParams(handle, pIn1, NULL, pM, pOut);
         currentTestFail        = ((status_nat != DSPLIB_ERR_NULL_POINTER) || (status_opt != DSPLIB_ERR_NULL_POINTER));
         break;
      ...
      case 1005:
         ...
         status_nat             = DSPLIB_w_vec_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_nat             = DSPLIB_w_vec_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_nat             = DSPLIB_w_vec_exec_checkParams(handle, pIn1, pIn2, NULL, pOut);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_w_vec_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_opt             = DSPLIB_w_vec_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         status_opt             = DSPLIB_w_vec_exec_checkParams(handle, pIn1, pIn2, pM, NULL);
         currentTestFail        = ((status_nat != DSPLIB_ERR_NULL_POINTER) || (status_opt != DSPLIB_ERR_NULL_POINTER));
         break;
      case 1006:
         bufParamsIn.data_type  = DSPLIB_INT128;
