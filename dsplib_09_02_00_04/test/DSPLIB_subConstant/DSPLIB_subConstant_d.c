...
#include "DSPLIB_subConstant_idat.h"
...
int DSPLIB_subConstant_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   int32_t              tpi;
   ...
   subConstant_testParams_t *prm;
   subConstant_testParams_t  currPrm;
   subConstant_getTestParams(&prm, &test_cases);

   DSPLIB_subConstant_InitArgs kerInitArgs;

#if DSPLIB_DEBUGPRINT
   printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 0\n");
#endif
...
#if DSPLIB_DEBUGPRINT
   printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 1\n");
#endif

   TI_profile_init("DSPLIB_subConstant");

   // file IO for Loki benchmarking
   FILE *fpOutputCSV = fopen("DSPLIB_subConstant.csv", "w+");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
...
#if DSPLIB_DEBUGPRINT
         printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d data_type %d dataSize %d \n", bufParamsIn.data_type,
                kerInitArgs.dataSize);
         printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d pIn1  %p pIn2 %p\n", pIn1, pIn2);
#endif
         ...
         if (pIn1 && pIn2 && pOut && pOutCn) {
            ...
            status_init = DSPLIB_subConstant_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 1 status_init %d\n", status_init);
#endif
            if (status_init == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               DSPLIB_asm(" MARK 0");
               kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
               status_init           = DSPLIB_subConstant_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
               DSPLIB_asm(" MARK 1");
               TI_profile_stop();
            }
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 2 status_init %d\n", status_init);
#endif
            status_opt = DSPLIB_subConstant_exec_checkParams(handle, pIn1, pIn2, pOut);
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 3 status_opt %d\n", status_opt);
#endif
            if (status_opt == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               DSPLIB_asm(" MARK 2");
               status_opt = DSPLIB_subConstant_exec(handle, pIn1, pIn2, pOut);
               DSPLIB_asm(" MARK 3");
               TI_profile_stop();
            }
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 4 status_opt %d\n", status_opt);
#endif
            // initialize the kernel to use the natural C variant
            DSPLIB_subConstant_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 5\n");
#endif
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            DSPLIB_asm(" MARK 8");
            status_nat = DSPLIB_subConstant_exec(handle, pIn1, pIn2, pOutCn);
            DSPLIB_asm(" MARK 9");
            TI_profile_stop();
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 6 status_nat %d\n", status_nat);
#endif
            if (currPrm.dataType == DSPLIB_FLOAT32 || currPrm.dataType == DSPLIB_FLOAT64) {
            ...
            comparisonDone = 1;
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 7 comparisonDone %d status_nat_vs_opt %d\n",
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 8 status_nat_vs_opt %d status_ref_vs_opt %d "
                   "currentTestFail %d\n",
                   status_nat_vs_opt, status_ref_vs_opt, currentTestFail);
            printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 8 status_init %d status_opt %d status_nat %d\n",
...
#if DSPLIB_DEBUGPRINT
            printf("DSPLIB_DEBUGPRINT  DSPLIB_subConstant_d CP 8 fail %d\n", fail);
#endif
            ...
            // write to CSV, must happen prior to write to screen because TI_profile_formula_subConstant clears values
            // in counters
            fprintf(fpOutputCSV, "subConstant, %d, %d, %d, Data size:%d, %d, %d, %d, %lu, %lu, %.2f, %d\n", testNum,
            ...
            // clear the counters between runs; normally handled by TI_profile_subConstant_test
...
int test_main(uint32_t *pProfile)
{
   ...
   if (TI_cache_init()) {
      TI_memError("DSPLIB_subConstant");
      return 1;
   }
   else
#else
...
#endif
   {
      return DSPLIB_subConstant_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   while (testNum <= 1006) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_subConstant_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_subConstant_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1001:
         ...
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat            = DSPLIB_subConstant_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt            = DSPLIB_subConstant_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1002:
         ...
         break;
      case 1003:
         status_nat      = DSPLIB_subConstant_exec_checkParams(handle, NULL, pIn2, pOut);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1004:
         status_nat      = DSPLIB_subConstant_exec_checkParams(handle, pIn1, NULL, pOut);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1005:
         status_nat      = DSPLIB_subConstant_exec_checkParams(handle, pIn1, pIn2, NULL);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1006:
         bufParamsIn.data_type  = DSPLIB_INT128;
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_subConstant_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_subConstant_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         currentTestFail        = ((status_nat != DSPLIB_ERR_INVALID_TYPE) || (status_opt != DSPLIB_ERR_INVALID_TYPE));
         break;
      default:
         break;
...
/* Main call for insubConstantidual test projects */
...
/* ======================================================================== */
/*  End of file:  DSPLIB_subConstant_d.c                                    */
/* ======================================================================== */
