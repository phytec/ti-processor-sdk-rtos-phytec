...
int DSPLIB_blk_move_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   blk_move_testParams_t *prm;
   blk_move_testParams_t  currPrm;
   blk_move_getTestParams(&prm, &test_cases);
   ...
   TI_profile_init("DSPLIB_blk_move");
#if !defined(EVM_TEST)
   FILE *fpOutputCSV = fopen("DSPLIB_blk_move.csv", "w+");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (pIn && pOut && pOutCn) {
            ...
            status_init           = DSPLIB_blk_move_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
            if (status_init == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               DSPLIB_asm(" MARK 0");
               kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
               status_init           = DSPLIB_blk_move_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
               DSPLIB_asm(" MARK 1");
               TI_profile_stop();
            }

            status_opt = DSPLIB_blk_move_exec_checkParams(handle, pIn, pOut);

            if (status_opt == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               DSPLIB_asm(" MARK 2");
               status_opt = DSPLIB_blk_move_exec(handle, pIn, pOut);
               DSPLIB_asm(" MARK 3");
               TI_profile_stop();
            }
            ...
            DSPLIB_asm(" MARK 8");
            status_nat = DSPLIB_blk_move_exec(handle, pIn, pOutCn);
            DSPLIB_asm(" MARK 9");
            TI_profile_stop();
            ...
            fprintf(fpOutputCSV, "blk_move, %d, %d, %d, -, %d, 1, 1, %d, %d, %3.2f\n", testNum, currPrm.testPattern,
...
#endif
   {
      return DSPLIB_blk_move_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   while (testNum <= 1005) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_blk_move_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_blk_move_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1001:
         ...
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_blk_move_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_blk_move_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1002:
         ...
         break;
      case 1003:
         status_nat      = DSPLIB_blk_move_exec_checkParams(handle, NULL, pOut);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1004:
         status_nat      = DSPLIB_blk_move_exec_checkParams(handle, pIn, NULL);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1005:
         bufParamsIn.data_type  = DSPLIB_INT128;
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_blk_move_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_blk_move_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         currentTestFail        = ((status_nat != DSPLIB_ERR_INVALID_TYPE) || (status_opt != DSPLIB_ERR_INVALID_TYPE));
         break;
      default:
         break;
...
/* Main call for inblk_moveidual test projects */
...
/* ======================================================================== */
/*  End of file:  DSPLIB_blk_move_d.c                          */
/* ======================================================================== */
