...
ifeq ($($(_MODULE)_TYPE),library)
	NEEDS_COMPILER=1
	CONCERTO_TARGETS_LIBS += $(if $(filter $(CONCERTO_TARGETS_LIBS),$(TARGET)),,$($(_MODULE)_TARGET))
	# no install for libs
else ifeq ($($(_MODULE)_TYPE),dsmo)
	NEEDS_COMPILER=1
	CONCERTO_TARGETS_DSMOS += $(if $(filter $(CONCERTO_TARGETS_DSMOS),$(TARGET)),,$($(_MODULE)_TARGET))
else ifeq ($($(_MODULE)_TYPE),exe)
	NEEDS_COMPILER=1
	CONCERTO_TARGETS_EXES += $(if $(filter $(CONCERTO_TARGETS_EXES),$(TARGET)),,$($(_MODULE)_TARGET))
...
else ifeq ($(HOST_COMPILER),NVCC_LINUX)
	include $(CONCERTO_ROOT)/compilers/nvcc_linux.mak
