...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    idatFile.write("%d, // count \n" % ((count)))
    idatFile.write("%d, // offset \n" % ((offset)))
...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_tableLookup_idat.h"'
    ...
    idatFile.write("static VXLIB_tableLookup_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def process_test_case(testCase):
    ...
    testCase["count"] = int(testCase["count"])
    testCase["offset"] = int(testCase["offset"])
...
def gen_test_case_header_file(testCase):
    ...
    [minValLut, maxValLut] = VXLIB_utils.resolveDTypeLimits(dType)
    # maxSrcIdx = ((count - offset - 1) > (maxValLut - 1))? ( maxValLut - 1):(count - offset - 1);
    maxSrcIdx = ( maxValLut ) if ((count - offset - 1) > (maxValLut)) else (count - offset - 1);
    if dType == "uint8_t":
      [minValIn, maxValIn] = [0, maxSrcIdx] 
      inp = np.random.uniform(low=minValIn, high=maxValIn, size=(height, width)).astype(
          np.uint8
      )
      lut = np.random.uniform(low=minValLut, high=maxValLut, size=(count)).astype(
          np.uint8
      )
    elif dType == "int8_t":
      [minValIn, maxValIn] = [- offset, maxSrcIdx] 
    ...
    file_io.write_header_file(inp, lut, out, testCase, headerFileName)
...
idatFileName = "../../VXLIB_tableLookup/VXLIB_tableLookup_idat.c"
##########################################################################
