...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    range = currPrm["range"]
    numBins = currPrm["numBins"]
    lastBlock = currPrm["lastBlock"]
    ...
    idatFile.write("%s, // range \n" % ((range)))
    idatFile.write("%s, // numBins \n" % ((numBins)))
    idatFile.write("%s, // final call to image \n" % ((lastBlock)))
...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_histogram_idat.h"'
    ...
    idatFile.write("static VXLIB_histogram_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def process_test_case(testCase):
    ...
    testCase["range"] = int(testCase["range"])
    testCase["numBins"] = int(testCase["numBins"])
    testCase["lastBlock"] = int(testCase["lastBlock"])
...
def gen_test_case_header_file(testCase):
    ...
    rangeHist = testCase["range"]
    numBins = testCase["numBins"]
    ...
    # inp = np.array(
    #     [[1, 2, 3, 4, 9, 10, 11, 12], [9, 10, 11, 12, 13, 14, 15, 16]]
    # ).astype(np.int32)

    out = np.zeros((numBins)).astype(np.uint32)

    for j in range(height):
        for i in range(width):
            pixel = inp[j][i].astype(np.uint8)
            if (offset <= pixel) and (pixel < (offset + rangeHist)):
                index = ((pixel - offset) * numBins // rangeHist).astype(np.uint32)
...
idatFileName = "../../VXLIB_histogram/VXLIB_histogram_idat.c"
