...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_gaussian_idat.h"'
    ...
    idatFile.write("static VXLIB_gaussian_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    Q_BITS = np.uint8(15)
    Q_FACTOR = np.uint32(1 << Q_BITS)

    if filterDim is 3:
        gaussianFilter = np.array([[1, 2, 1], [2, 4, 2], [1, 2, 1]])
        floatCoeff = 1 / 16
        gaussianFilterQ = np.uint16(Q_FACTOR * (gaussianFilter * floatCoeff))

        # c7100 implementation without any padding
        if isNotPadded:
                ...
                for i in range(outWidth):
                    ...
                    for n in range(filterDim):
                        ...
                        for m in range(filterDim):
                            ...
                            sum += np.uint32(inp[j + n][i + m] * gaussianFilterQ[n][m])

                    result = np.uint32(sum >> Q_BITS)
                ...
                for i in range(outWidth):
                    ...
                    for n in range(filterDim):
                        ...
                        for m in range(filterDim):
                            ...
                            sum += np.uint32(inp[j + n][i + m] * gaussianFilter[n][m])

                    result = np.uint32(((sum) >> shift))
                    out[j][i] = np.uint8(result)
...
idatFileName = "../../VXLIB_gaussian/VXLIB_gaussian_idat.c"
