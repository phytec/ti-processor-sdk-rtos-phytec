...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_normL2_idat.h"'
    ...
    idatFile.write("static VXLIB_normL2_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    sqrin0 = np.array(list(map(lambda x: [y**2 for y in x], in0)))
    sqrin1 = np.array(list(map(lambda x: [y**2 for y in x], in1)))
    sqrRoot = np.sqrt(sqrin0 + sqrin1)
    
    sqrRootRound = np.array(list(map(lambda x: [ (y + 0.5) if (int(y) % 2  == 1) and (y-int(y) == 0.5) else y+0.5  for y in x], sqrRoot)))

    out =  np.array(list(map(lambda x: [ int(y)  for y in x], sqrRootRound)))
...
idatFileName = "../../VXLIB_normL2/VXLIB_normL2_idat.c"
