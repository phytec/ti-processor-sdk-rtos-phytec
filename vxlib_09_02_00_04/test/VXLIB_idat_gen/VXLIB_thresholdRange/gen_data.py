...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    upperVal              = currPrm["upperVal"]
    lowerVal              = currPrm["lowerVal"]
    ...
    idatFile.write("%d, // upperVal of image \n" % ((upperVal)))
    idatFile.write("%d, // lowerVal of image \n" % ((lowerVal)))
...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_thresholdRange_idat.h"'
    ...
    idatFile.write("static VXLIB_thresholdRange_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def process_test_case(testCase):
    ...
    testCase["upperVal"] = int(testCase["upperVal"])
    testCase["lowerVal"] = int(testCase["lowerVal"])
...
def gen_test_case_header_file(testCase):
    ...
    upperVal        = testCase["upperVal"]
    lowerVal        = testCase["lowerVal"]
    ...
    out = np.array(list(map(lambda x: [ falseVal if (y > upperVal) or (y < lowerVal) else trueVal  for y in x], inp)))
    
    # out = [ falseVal if x > upperVal else falseVal for x in inp ]
...
idatFileName = "../../VXLIB_thresholdRange/VXLIB_thresholdRange_idat.c"
