...
def gen_idat_file(idatFile, testCases, alphaList):
    ...
    includeString = '#include "VXLIB_accumulateWeightImage_idat.h"'
    ...
    idatFile.write("static VXLIB_accumulateWeightImage_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    in1 = np.random.uniform(low=minValOut, high=maxValOut, size=(height, width)).astype(
        np.uint8
    )
    ...
    vOut = np.trunc((alpha * in0 + (1 - alpha) * in1)).astype(np.uint8)


    headerFileName = "staticRefCase" + str(testId) + ".h"
    file_io.write_header_file(in0, in1, vOut, testCase, headerFileName)

    return [np.min(vOut), np.max(vOut), alpha]
...
idatFileName = "../../VXLIB_accumulateWeightImage/VXLIB_accumulateWeightImage_idat.c"
