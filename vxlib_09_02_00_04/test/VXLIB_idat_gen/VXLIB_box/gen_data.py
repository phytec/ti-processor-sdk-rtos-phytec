...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_box_idat.h"'
    ...
    idatFile.write("static VXLIB_box_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    boxFilter = np.ones((filterDim, filterDim))
    ...
    floatCoeff = 1.0/(filterDim * filterDim)
    Q_BITS = np.uint8(15)
    Q_FACTOR = np.uint32(1 << Q_BITS)
    fixedCoeff = np.uint16(Q_FACTOR * floatCoeff)
            ...
            for i in range(outWidth):
                ...
                for n in range(filterDim):
                    ...
                    for m in range(filterDim):
                        ...
                        sum += np.uint32(inp[j+n][i+m] * fixedCoeff)

                result = np.uint32(sum >> Q_BITS)
                out[j][i] = np.uint8(result)

    # TODO: c7120, c7504 implementation with padding
...
testIdList = range(1,12)  # list of test IDs that we want to generate test data
...
idatFileName = "../../VXLIB_box/VXLIB_box_idat.c"
