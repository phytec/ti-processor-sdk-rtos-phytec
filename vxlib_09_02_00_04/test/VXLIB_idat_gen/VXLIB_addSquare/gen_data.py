...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_addSquare_idat.h"'
    ...
    idatFile.write("static VXLIB_addSquare_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    vSqr = in0 * in0
    vOut = np.right_shift(vSqr, shift, out=vSqr, where=shift != 0) + in1

    vOut = np.clip(vOut, a_min=-32768, a_max=32767)

    # np.add(outVec, 1, out=outVec, where=shift != 0)
    # np.right_shift(outVec, 1, out=outVec, where=shift != 0)

    headerFileName = "staticRefCase" + str(testId) + ".h"
    file_io.write_header_file(in0, in1, vOut, testCase, headerFileName)

    return [np.min(vOut), np.max(vOut)]
...
idatFileName = "../../VXLIB_addSquare/VXLIB_addSquare_idat.c"
