...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_multiply_idat.h"'
    ...
    idatFile.write("static VXLIB_multiply_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def process_test_case(testCase):
    ...
    testCase["scale"] = float(testCase["scale"])
...
def gen_test_case_header_file(testCase):
    ...
    in0 = np.random.uniform(low=minValIn0, high=maxValIn0, size=(height, width)).astype(
          VXLIB_utils.resolveNumpyDType(dTypeIn0))
    
    in1 = np.random.uniform(low=minValIn1, high=maxValIn1, size=(height, width)).astype(
            VXLIB_utils.resolveNumpyDType(dTypeIn1))

    scale = np.float32(scale)
    print("scale: ", scale, type(scale))

    out   = np.multiply(in0.astype(np.int32) , in1.astype(np.int32)).astype(np.int32)
    
    out_scaled = scale * (out.astype(np.float64)) 
  
    
    if dTypeIn0 == "int16_t" and dTypeIn1 == "int16_t" and dTypeOut == "int16_t":
        out_scaled_int = out_scaled.astype((np.int64))
    else:
        out_scaled_int = out_scaled.astype((np.int32))

    if overFlowPolicy == "VXLIB_WRAP":
        out_final = out_scaled_int.astype(
            VXLIB_utils.resolveNumpyDType(dTypeOut))

    else:
        out_final = np.clip(out_scaled_int, a_min=minValOut, a_max=maxValOut).astype(VXLIB_utils.resolveNumpyDType(dTypeOut))

    headerFileName = "staticRefCase" + str(testId) + ".h"
    file_io.write_header_file(in0, in1, out_final, testCase, headerFileName)
    return [np.min(out_final), np.max(out_final)]
...
idatFileName = "../../VXLIB_multiply/VXLIB_multiply_idat.c"
