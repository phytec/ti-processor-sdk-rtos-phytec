...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    idatFile.write("%s, // data type in \n" % (VXLIB_utils.resolveVXLIBDType(dTypeIn)))
    idatFile.write("%s, // Output vector \n" % ((out)))
    idatFile.write(
        "%s, // data type out \n" % (VXLIB_utils.resolveVXLIBDType(dTypeOut))
    )
    idatFile.write("%d, // width of input image \n" % ((widthIn)))
    idatFile.write("%d, // height of input image \n" % ((heightIn)))
    idatFile.write("%d, // width of output image \n" % ((widthOut)))
    idatFile.write("%d, // height of output image \n" % ((heightOut)))
    idatFile.write("%d, // size of filter \n" % ((filterDim)))
...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_halfScaleGaussian_idat.h"'
    ...
    idatFile.write("static VXLIB_halfScaleGaussian_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def process_test_case(testCase):
    ...
    testCase["filterDim"] = int(testCase["filterDim"])
...
def gen_test_case_header_file(testCase):
    ...
    inp = np.random.uniform(
        low=minValIn, high=maxValIn, size=(heightIn, widthIn)
    ).astype(np.int64)

    isNotPadded = (
        (padLeft == 0) and (padRight == 0) and (padTop == 0) and (padBottom == 0)
    )
    ...
    out = np.zeros((outHeight, outWidth)).astype(np.uint8)

    gaussianFilter = np.uint16(
        np.array(
            [
                [1, 4, 6, 4, 1],
                [4, 16, 24, 16, 4],
    ...
    if isNotPadded:
        ...
        for j in range(outHeight):
            for i in range(outWidth):

                sum = np.uint32(0)

                for n in range(filterDim):

                    for m in range(filterDim):

                        sum += np.uint32(
                            inp[(2 * j) + n][(2 * i) + m] * gaussianFilter[n][m]
                        )

                result = np.uint32(((sum >> 8)))
                out[j][i] = np.uint8(result)
...
testIdList = range(1, 25)  # list of test IDs that we want to generate test data
# testIdList = [2]
...
idatFileName = "../../VXLIB_halfScaleGaussian/VXLIB_halfScaleGaussian_idat.c"
