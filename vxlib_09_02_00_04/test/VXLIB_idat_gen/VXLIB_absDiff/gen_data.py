...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    idatFile.write(
        "%s * sizeof(%s), // stride of image 0 \n" % ((strideIn0), (dTypeIn0))
    )
    idatFile.write(
        "%s * sizeof(%s), // stride of image 1 \n" % ((strideIn1), (dTypeIn0))
    )
    idatFile.write(
        "%s * sizeof(%s), // stride of output image \n" % ((strideOut), (dTypeIn0))
    )
...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_absDiff_idat.h"'
    ...
    idatFile.write("static VXLIB_absDiff_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    if dTypeIn0 == "int16_t":
        ...
        in1 = np.random.uniform(low=minVal, high=maxVal, size=(height, width)).astype(
            "int32"
        )
    elif dTypeIn0 == "uint8_t":
        in0 = np.absolute(
            np.random.uniform(low=minVal, high=maxVal, size=(height, width)).astype(
                "int16"
            )
        )
        in1 = np.absolute(
    ...
    if dTypeIn0 == "uint8_t":
        out = (np.absolute((in0 - in1))).astype("uint8")
        in0 = in0.astype("uint8")
        in1 = in1.astype("uint8")
    elif dTypeIn0 == "int16_t":
        out = np.absolute((in0 - in1))
        out[out > np.iinfo(np.int16).max] = np.iinfo(np.int16).max
        out[out < np.iinfo(np.int16).min] = np.iinfo(np.int16).min
        out = out.astype("int16")
        in0 = in0.astype("int16")
        in1 = in1.astype("int16")
    else:
        out = np.absolute((in0 - in1))
...
idatFileName = "../../VXLIB_absDiff/VXLIB_absDiff_idat.c"
