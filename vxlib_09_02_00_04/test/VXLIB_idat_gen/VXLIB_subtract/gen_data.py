...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    subtractPolicy = currPrm["subtractPolicy"]
    ...
    idatFile.write("%s, // subtractPolicy \n" % ((subtractPolicy)))
...
def gen_idat_file(idatFile, testCases):
    ...
    includeString = '#include "VXLIB_subtract_idat.h"'
    ...
    idatFile.write("static VXLIB_subtract_testParams_t testParams[] =  ")
    idatFile.write("{\n")
...
def process_test_case(testCase):
    ...
    testCase["subtractPolicy"] = int(testCase["subtractPolicy"])
...
def gen_test_case_header_file(testCase):
    ...
    subtractPolicy = testCase["subtractPolicy"]
    ...
    if ((dTypeIn0 == "uint8_t") and (dTypeIn1 == "int16_t") and (dTypeOut == "int16_t")):
        if (subtractPolicy == 0):
            out = (in0 - in1).astype(np.int64)
        else:
            out = (in1 - in0).astype(np.int64)
    else:
        out = (in0 - in1).astype(np.int64)

    
    if overFlowPolicy == "VXLIB_WRAP":
        out = out.astype(VXLIB_utils.resolveNumpyDType(dTypeOut))

    else:
        # out = (in0 + in1).astype(np.int64)
        out = np.clip(out, a_min=minValOut, a_max=maxValOut).astype(
...
idatFileName = "../../VXLIB_subtract/VXLIB_subtract_idat.c"
