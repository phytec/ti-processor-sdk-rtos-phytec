 ...
 *	static uint32_t flint32_t(uint32_t i)
 *	{
 *		assert(i != 0);
...
#define builtin_ilog32_nz(v) \
  ((u32)( (s32) (((int) sizeof(unsigned) * CHAR_BIT) - (int)__builtin_clz(v))))
...
# define STATIC_ILOG2(_v) (((_v) & \
			    0xC) ? 2 + \
			   STATIC_ILOG1((_v) >> 2) : STATIC_ILOG1(_v))
...
# define STATIC_ILOG6(_v) \
	(((_v) & \
	 ...
	 STATIC_ILOG5((_v) >> 32) : STATIC_ILOG5(_v))
