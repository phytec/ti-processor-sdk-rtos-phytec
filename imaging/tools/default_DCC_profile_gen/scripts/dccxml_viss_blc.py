...
def GenVISS_BLCParams(handle, VISS_BLC_params, cls_id, tuning_params):

    handle.write('   				 //viss_clamp_vshort : For linear mode only this should be set to sensor Black Level\n')
    handle.write('   				  {\n')
    
    blc = 0
    if tuning_params['WDR_MODE'] == 'linear':
        blc = tuning_params['BLACK_POST']

    handle.write('   						{}, // dcoffset_clamp_value 0\n'.format(-blc))
    handle.write('   						{}, // dcoffset_clamp_value 1\n'.format(-blc))
    handle.write('   						{}, // dcoffset_clamp_value 2\n'.format(-blc))
    handle.write('   						{}, // dcoffset_clamp_value 3\n'.format(-blc))
