...
static word add_sp(word op1, word op2, int cr_wr_delay)
{
    ...
    if      ( rc == 1 ) return( 0x7fffffff ) ;    /* QNaN */
    else if ( rc == 2 ) return( 0x7f800000 ) ;    /* +infinity */
    else if ( rc == 3 ) return( 0xff800000 ) ;    /* -infinity */
    ...
    if ( !exp1 ) {  	                                      /* test op1 == zero (or denormal)          */
      if ( !exp2 ) {                                              /*  both operands are zero                 */
        if ( sign1 == sign2 ) return( sign1 ? 0x80000000 : 0 ) ; /*  if same sign, return that sign of zero */
        else {                                                    /*  for different signed zeros             */
          if ( round_mode == 3 ) return( 0x80000000 ) ;           /*    return -zero if round down           */
          else return( 0 ) ;                                      /*    return +zero for other roundings     */
        }
      }
      else return( op2 ) ;                                        /*  return op2 if op1==0 && op2!=0         */
    }
    if ( !exp2 ) return( op1 ) ;                                  /*  return op1 if op2==0 && op1!=0         */
    ...
    if ( exp1 > exp2 ) {
      ...
      result_sign = sign1 ;
      exp_diff = exp1 - exp2 ;
      if ( exp_diff > 31 ) man2 = 1 ;                  /* just sticky bit is set */
      else {
      ...
      if ( ( man1 == man2 ) && ( sign1 != sign2 ) ) {
        if ( round_mode == 3 ) return( 0x80000000 ) ;
        else                   return( 0 ) ;
      }
      else {
        result_exp = exp1 ;
...
static dword ccmpy32r1(dword op1, dword op2)
{
    ...
    dword mpy_res ;
      ...
      src1_real = op1.high ; src1_real <<= 32 ; src1_real >>= 32 ;
      src1_im   = op1.low  ; src1_im   <<= 32 ; src1_im   >>= 32 ;  
      src2_real = op2.high ; src2_real <<= 32 ; src2_real >>= 32 ; 
      src2_im   = op2.low  ; src2_im   <<= 32 ; src2_im   >>= 32 ;  
...
static word cmpgt2(word a,word b)
{
    word l1,l2,h1,h2,c;
       ...
       l1 = 0xffff & a;
       if (0x8000 & l1) l1 |= 0xffff0000;
       h1 = 0xffff & (a>>16);
       if (0x8000 & h1) h1 |= 0xffff0000;
       l2 = 0xffff & b;
       if (0x8000 & l2) l2 |= 0xffff0000;
       h2 = 0xffff & (b>>16);
       if (0x8000 & h2) h2 |= 0xffff0000;
...
static word expand2(word a)
{
    ...
    result = 0;
    if (a & 1) result |= 0xffff;
    if (a & 2) result |= 0xffff0000;
...
static word expand4(word a)
{
    ...
    result = 0;
    if (a & 1) result |= 0xff;
    if (a & 2) result |= 0xff00;
    if (a & 4) result |= 0xff0000;
    if (a & 8) result |= 0xff000000;
...
static word intsp(word op, int cr_wr_delay)
{
    ...
    if ( man.low ) {
      if ( datapath == a_path ) FADCR.INEXADD1 |= 1 ; // cr_wr_delay delay slots
...
static word max2(word a,word b)
{
    ...
    c = 0;
    alo = 0xffff & a;
    if (alo & 0x8000) alo |= 0xffff0000;
    blo = 0xffff & b;
    if (blo & 0x8000) blo |= 0xffff0000;
    if (alo > blo) c |= a & 0xffff;
    else           c |= b & 0xffff;
...
static word mpy_sp(word op1, word op2)
{
    ...
    if ( rc == 1 ) return( result_sign ? 0xffffffff : 0x7fffffff ) ;
    else if ( rc == 2 ) return( result_sign ? 0xff800000 : 0x7f800000 ) ;
    
    exp1 = extract_exp_sp( op1 ) ;
    exp2 = extract_exp_sp( op2 ) ;
    
    if ( !exp1 || !exp2 ) return( result_sign ? 0x80000000 : 0 ) ; /* one operand is zero or denormal */
...
static word round_ofuf_chk_sp(word sign, word exp, word man, word round, word sticky, word adder, int cr_wr_delay)
{
    ...
    if ( adder ) round_mode = ( datapath == a_path ) ? FADCR.ROUNDINGMODEADD1 : FADCR.ROUNDINGMODEADD2 ; 
...
static word sadd2(word a,word b)
{
    ...
    a_low = a & 0xffff;
    b_low = b & 0xffff;
    if (a_low & 0x8000) a_low = a_low | 0xffff0000;
    if (b_low & 0x8000) b_low = b_low | 0xffff0000;
    d_low  =  a_low + b_low;               /* Add the low 16-bits */
    if (d_low > (int) 0x7fff) d_low = 0x7fff;
    else if (d_low < (int) 0xffff8000) d_low = 0x8000;
    
    d_low &= 0xFFFF; /* mask off upper bits */
    
    a_high = a>>16;
    b_high = b>>16;
    if (a_high & 0x8000) a_high = a_high | 0xffff0000;
    if (b_high & 0x8000) b_high = b_high | 0xffff0000;
    d_high  =  a_high + b_high;               /* Add the high 16-bits */
    if (d_high > (int) 0x7fff) d_high = 0x7fff;
    else if (d_high < (int) 0xffff8000) d_high = 0x8000;
...
static word sat_check32(int64_ll a)
{
    int i ; 
       ...
       i = 0x0 ;
       if (a >= ((int64_ll) 0x80000000)) i = 0x1 ;
       if (a < -((int64_ll) 0x80000000)) i = 0x1 ;
...
static word sat_jz(int64_ll a)
{
    int i ;
       ...
       i = a ;
       if (a >= ((int64_ll) 0x80000000)) i = 0x7FFFFFFF;
       if (a < -((int64_ll) 0x80000000)) i = 0x80000000;
...
{
    word d0,d1,dst;
        if      (a > (int)0x00007fff) d0 = 0x7fff;
        else if (a < (int)0xffff8000) d0 = 0x8000;
        else                          d0 = a & 0xffff;
    
        if      (b > (int)0x00007fff) d1 = 0x7fff;
        else if (b < (int)0xffff8000) d1 = 0x8000;
        else                          d1 = b & 0xffff;
...
static word spack4(word a, word b)
{
    word x0,x1,x2,x3,d0,d1,d2,d3,dst;
     ...
     d0 = x0 & 0xff;
     if (x0 & 0xff00) 
       if (x0 & 0x8000) d0 = 0; else d0 = 0xff;
    
     d1 = x1 & 0xff;
     if (x1 & 0xff00) 
       if (x1 & 0x8000) d1 = 0; else d1 = 0xff;
    
     d2 = x2 & 0xff;
     if (x2 & 0xff00) 
       if (x2 & 0x8000) d2 = 0; else d2 = 0xff;
    
     d3 = x3 & 0xff;
     if (x3 & 0xff00) 
       if (x3 & 0x8000) d3 = 0; else d3 = 0xff;
...
static word ssub2_joule(word a, word b)
{
     ...
     a_lo   = slow16(a) ; a_hi = shigh16(a) ;
     b_lo   = slow16(b) ; b_hi = shigh16(b) ;
    
     u_tmp  = a_hi - b_hi ;
    
     // check to see if the result saturated
    
     if (  (a_hi & 0x8000) &&  !(b_hi & 0x8000) && !(u_tmp & 0x8000) )  u_tmp = 0x8000 ;
     if ( !(a_hi & 0x8000) &&   (b_hi & 0x8000) &&  (u_tmp & 0x8000) )  u_tmp = 0x7fff ;
     ...
     if (  (a_lo & 0x8000) &&  !(b_lo & 0x8000) && !(u_tmp & 0x8000) )  u_tmp = 0x8000 ;
     if ( !(a_lo & 0x8000) &&   (b_lo & 0x8000) &&  (u_tmp & 0x8000) )  u_tmp = 0x7fff ;
...
static dword rcp_dp(dword op)
{
    ...
    if ( ( op.high & 0x000fffff ) || op.low ) {
      if ( datapath == a_path ) FAUCR.INEXAU1 |= 1 ;
...
static word rcp_sp(word op)
{
    ...
    if ( inf_chk_sp( op ) ) return( sign ? 0x80000000 : 0 ) ;
    man = extract_man_sp( op ) ;
