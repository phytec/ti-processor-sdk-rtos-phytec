...
#include "FFTLIB_FFT_fft_ixX_cxX_oxX_idat.h"


__attribute__ ((section (".ddrData"), aligned (64))) int8_t ddrBuffer[2048 * 1024];
int16_t volatile volatileSum = 0;  // use volatile to keep compiler from removing this operation

int FFTLIB_FFT_fft_ixX_cxX_oxX_d(
   uint32_t *pProfile,
   uint8_t LevelOfFeedback)
{
   ...
   FILE *fpOutputCSV = fopen("FFTLIB_FFT_fft_ixX_cxX_oxX.csv", "w+");
   ...
   FFTLIB_FFT_fft_ixX_cxX_oxX_InitArgs    kerInitArgs;

   int32_t handleSize = FFTLIB_FFT_fft_ixX_cxX_oxX_getHandleSize(&kerInitArgs);
   FFTLIB_kernelHandle handle = malloc(handleSize);

   TI_profile_init("FFTLIB_FFT_fft_ixX_cxX_oxX");
   ...
   for (tpi=0; tpi < test_cases; tpi++) {
	 ...
	 for (k = 0; k < FFTLIB_FFT_FFT_IXX_CXX_OXX_NUMSHIFTS; k++) {
	 ...
	 FFTLIB_FFT_fft_ixX_cxX_oxX_getSizes(&kerInitArgs,
					     &bufParamsX,
					     &bufParamsW,
					     &bufParamsY);
...
//	 void *pYCn = (void *) malloc(bufParamsX.dim_x*FFTLIB_sizeof(currPrm.dataType));
	 ...
	 if( pX && pW && pY && pYCn ) {
	    ...
	    FFTLIB_FFT_fft_ixX_cxX_oxX_twGen(&kerInitArgs,
					     pW,
					     &bufParamsW);

	    status_init = FFTLIB_FFT_fft_ixX_cxX_oxX_init_checkParams(handle,
								      &bufParamsX,
								      &bufParamsW,
								      &bufParamsY,
								      &kerInitArgs);
	    if (status_init == FFTLIB_SUCCESS) {
	       ...
	       status_init = FFTLIB_FFT_fft_ixX_cxX_oxX_init(handle,
							     &bufParamsX,
							     &bufParamsW,
							     &bufParamsY,
							     &kerInitArgs);
	    ...
	    status_opt = FFTLIB_FFT_fft_ixX_cxX_oxX_exec_checkParams(handle,
								     pX,
								     pW,
								     pY);
	    if(status_opt == FFTLIB_SUCCESS){
	       ...
	       FFTLIB_asm(" MARK 2");
	       status_opt = FFTLIB_FFT_fft_ixX_cxX_oxX_exec(handle,
							    pX,
							    pW,
							    pY);
	       FFTLIB_asm(" MARK 3");
	    ...
	    FFTLIB_FFT_fft_ixX_cxX_oxX_init(handle,
					    &bufParamsX,
					    &bufParamsW,
					    &bufParamsY,
					    &kerInitArgs);
	    ...
	    FFTLIB_asm(" MARK 8");
	    status_nat = FFTLIB_FFT_fft_ixX_cxX_oxX_exec(handle,
							 pX,
							 pW,
							 pYCn);
	    FFTLIB_asm(" MARK 9");
	    ...
	    fprintf(fpOutputCSV, "FFT wrapper, %d, %d, %d, FFT:%d Batch:%d Int/Deint: %d, %d, %d, %d, , %lu, , %d\n", testNum, currPrm.testPattern, FFTLIB_sizeof(bufParamsY.data_type)*8, currPrm.fftSize, currPrm.batchSize, currPrm.interleave, currPrm.fftSize, currPrm.batchSize, currPrm.interleave, cycles[TI_PROFILE_KERNEL_OPT], !currentTestFail);
#endif // #if ( !defined(RTL_TEST) && !defined(QT_TEST) && !defined(EVM_TEST))
...
int test_main(uint32_t *pProfile) {
   ...
   if( TI_cache_init()) {
      TI_memError("FFTLIB_FFT_fft_ixX_cxX_oxX");
      return 1;
   } else
#else
...
#endif
   {
      return FFTLIB_FFT_fft_ixX_cxX_oxX_d(&pProfile[0],
					  0);
   }
}
...
#endif
/* ======================================================================== */
/*  End of file:  FFTLIB_FFT_fft_ixX_cxX_oxX_d.c                          */
/* ======================================================================== */
