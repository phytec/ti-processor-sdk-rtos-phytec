...
int FFTLIB_fft1d_i32f_c32fc_o32fc_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   int32_t                              tpi;
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      pX = (FFTLIB_F32 *) TI_memalign(128,                                 /* pX is required to be*/
                                      numPoints * 2 * sizeof(FFTLIB_F32)); /* 16-byte aligned for
                                                                            * streaming engine use

                                                                            * in kernel           */
      pXFFT = (FFTLIB_F32 *) TI_memalign(128, (numPoints + 1) * 2 * sizeof(FFTLIB_F32));
      pY    = (FFTLIB_F32 *) TI_memalign(128, (numPoints + 1) * 2 * sizeof(FFTLIB_F32));
      pW    = (FFTLIB_F32 *) TI_memalign(128, numPoints * 2 * sizeof(FFTLIB_F32));
       ...
       * (FFTLIB_F32)); */
      /* #endif */
      pSf = (FFTLIB_F32 *) TI_memalign(128, (numPoints + 1) * 2 * sizeof(FFTLIB_F32));

      if (pX && pY && pW && pYCn && pSf && pXFFTCn) {
         FFTLIB_bufParams1D_t bufParamsData;
         if ((prm[tpi].testPattern == RANDOM) || (prm[tpi].testPattern == RANDOM_SIGNED)) {
            uint32_t i = 0;
            for (i = 0; i < numPoints * 2; i++) {
               pX[i] = (float) rand() / (float) (RAND_MAX / (1 << 8)) - (1 << 7);
            }
         }
         else {
            TI_fillBuffer(prm[tpi].testPattern, 0, pX, prm[tpi].staticIn0, numPoints * 2, 1, 1, sizeof(FFTLIB_F32),
                          testPatternString);
         }

         TI_fillBuffer(STATIC, 0, pXCn, pX, numPoints * 2, 1, 1, sizeof(FFTLIB_F32), testPatternString);

         bufParamsData.dim_x     = numPoints;
         bufParamsData.data_type = FFTLIB_FLOAT32;

         tw_gen_r2c(pW, numPoints);
         split_factor_gen_r2c(pSf, numPoints);

         TI_profile_start(TI_PROFILE_KERNEL_INIT);
         status_opt = FFTLIB_fft1d_i32f_c32fc_o32fc_init((FFTLIB_F32 *) pX, &bufParamsData, (FFTLIB_F32 *) pW,
         ...
         FFTLIB_asm(" MARK 0");
         status_opt = FFTLIB_fft1d_i32f_c32fc_o32fc_kernel((FFTLIB_F32 *) pX, &bufParamsData, (FFTLIB_F32 *) pW,
                                                           &bufParamsData, (FFTLIB_F32 *) pXFFT, &bufParamsData,
                                                           (FFTLIB_F32 *) pSf, &bufParamsData, (FFTLIB_F32 *) pY,
                                                           &bufParamsData, FFTLIB_fft1d_i32f_c32fc_o32fc_pBlock);
         FFTLIB_asm(" MARK 1");
         ...
         TI_fillBuffer(STATIC, 0, pX, pXCn, numPoints * 2, 1, 1, sizeof(FFTLIB_F32), testPatternString);
   ...
   not remove
         ...
         status_nat = FFTLIB_fft1d_i32f_c32fc_o32fc_cn(
             (FFTLIB_F32 *) pXCn, &bufParamsData, (FFTLIB_F32 *) pW, &bufParamsData, (FFTLIB_F32 *) pXFFTCn,
             &bufParamsData, (FFTLIB_F32 *) pSf, &bufParamsData, (FFTLIB_F32 *) pYCn, &bufParamsData);
         TI_profile_stop();

         status_nat_vs_opt = TI_compare_mem_2D_float((void *) pYCn, (void *) pY, 0, (double) powf(2, -3), numPoints * 2,
                                                     1, numPoints * 2, sizeof(FFTLIB_F32));

         if (prm[tpi].staticOut != NULL) {
            status_opt_vs_ref =
                TI_compare_mem_2D_float((void *) prm[tpi].staticOut, (void *) pY, 0, (double) powf(2, -3),
         ...
         currentTestFail = ((status_nat_vs_opt == TI_TEST_KERNEL_FAIL) || (status_opt_vs_ref == TI_TEST_KERNEL_FAIL) ||
         ...
         /* est_test = 1; */
         sprintf(desc, "%s generated input | numPoints=%d", testPatternString, numPoints);
         ...
         TI_profile_add_test(testNum++, numPoints, archCycles, estCycles, currentTestFail, desc);
      }
      else {
...
int main()
{
   ...
   for (i = 0; i < test_cases; i++) {
      printf("Test %4d: Cold Cycles = %8d, Warm Cycles = %8d, Warm Cycles WRB "
             "= %8d\n",
             i, profile[3 * i], profile[3 * i + 1] / NUM_RUNS, profile[3 * i + 2]);
   }
#endif
