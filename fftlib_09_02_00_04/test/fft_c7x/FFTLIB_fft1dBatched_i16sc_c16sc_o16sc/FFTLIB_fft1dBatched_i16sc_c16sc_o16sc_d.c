...
#pragma DATA_SECTION(FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_pBlock, ".pinit")
uint8_t FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_pBlock[FFTLIB_FFT1DBATCHED_I16SC_C16SC_O16SC_PBLOCK_SIZE];
...
void tw_gen(int16_t *pW, uint32_t numPoints)
{
   ...
   for (j = 1, k = 0; j <= numPoints >> 2; j = j << 2) {
      for (i = 0; i < numPoints >> 2; i += j) {
         ...
         pW[k + 2 * t]     = float2short(twF2sScale * cos(4 * PI * i / numPoints));
         pW[k + 2 * t + 1] = float2short(twF2sScale * (-sin(4 * PI * i / numPoints)));
         pW[k + 4 * t]     = float2short(twF2sScale * cos(6 * PI * i / numPoints));
         pW[k + 4 * t + 1] = float2short(twF2sScale * (-sin(6 * PI * i / numPoints)));
         k += 2;
      }
      k += (t) *4;
      t = t >> 2;
...
int FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   int32_t                                      tpi;
   ...
   TI_profile_init("FFTLIB_fft1dBatched_i16sc_c16sc_o16sc");

#if (!defined(RTL_TEST) && !defined(QT_TEST) && !defined(EVM_TEST))
   FILE *fpOutputCSV = fopen("FFTLIB_fft1dBatched_i16sc_c16sc_o16sc.csv", "w+");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      dataMemSize = numChannels * numPoints < 128 ? /* Kernel requires input/output */
                        128 * 2
                                                  : numChannels * numPoints * 2; /* buffers to be atleast
                                                                                  * 128 elements long */

      pX = (int16_t *) TI_memalign(128,                            /* pX is required to be*/
      ...
      if (pX && pY && pW && pXCn && pYCn && pShift) {
         ...
         TI_fillBuffer(prm[tpi].testPattern, 0, pX, prm[tpi].staticIn0, bufWidth, bufHeight, bufWidth * sizeof(int16_t),
         ...
         TI_fillBuffer(STATIC, 0, pXCn, pX, bufWidth, bufHeight, bufWidth * sizeof(int16_t), sizeof(int16_t),
                       testPatternString);

         bufParamsShift.dim_x     = numShifts;
         bufParamsShift.data_type = FFTLIB_UINT32;
         TI_fillBuffer(STATIC, 0, /* Shift vector is always STATIC */
                       pShift, prm[tpi].shiftVector, numShifts, 1, 1, sizeof(uint32_t), testPatternString);
         ...
         status_opt = FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_init(
             (int16_t *) pX, &bufParamsData, (int16_t *) pW, &bufParamsTw, (int16_t *) pY, &bufParamsData,
             (uint32_t *) pShift, &bufParamsShift, numPoints, numChannels,
             FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_pBlock);
         TI_profile_stop();

         TI_profile_start(TI_PROFILE_KERNEL_OPT);
         FFTLIB_asm(" MARK 0");
         status_opt = FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_kernel(
             (int16_t *) pX, &bufParamsData, (int16_t *) pW, &bufParamsTw, (int16_t *) pY, &bufParamsData,
             (uint32_t *) pShift, &bufParamsShift, numPoints, numChannels,
             FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_pBlock);
         FFTLIB_asm(" MARK 1");
          ...
          * writes into input buffer                                */
         TI_fillBuffer(STATIC, 0, pX, pXCn, bufWidth, bufHeight, bufWidth * sizeof(int16_t), sizeof(int16_t),
                       testPatternString);
...
not remove
         ...
         status_nat = FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_cn(
             (int16_t *) pXCn, &bufParamsData, (int16_t *) pW, &bufParamsTw, (int16_t *) pYCn, &bufParamsData,
             (uint32_t *) pShift, &bufParamsShift, numPoints, numChannels);
         TI_profile_stop();

         status_nat_vs_opt = TI_compare_mem((void *) pY, (void *) pYCn, numChannels * numPoints * 2 * sizeof(int16_t));
         if (prm[tpi].staticOut != NULL) {
            status_nat_vs_ref = TI_compare_mem((void *) prm[tpi].staticOut, (void *) pYCn,
                                               numChannels * numPoints * 2 * sizeof(int16_t));
         }

         currentTestFail = ((status_nat_vs_opt == TI_TEST_KERNEL_FAIL) || (status_nat_vs_ref == TI_TEST_KERNEL_FAIL) ||
         ...
         /* est_test = 1; */
         sprintf(desc, "%s generated input | numPoints=%d, numChannels=%d", testPatternString, numPoints, numChannels);

#if (!defined(RTL_TEST) && !defined(QT_TEST) && !defined(EVM_TEST))
         fprintf(fpOutputCSV, "%d, %d, %d\n", testNum, numChannels, numPoints);
#endif

         uint64_t archCycles = 0, estCycles = 0;
         TI_profile_add_test(testNum++, numPoints * numChannels, archCycles, estCycles, currentTestFail, desc);
      }
      else {
...
#endif //_HOST_BUILD
   {
      return FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_d(&pProfile[0], 0);
   }
}
...
/* ======================================================================== */
/*  End of file:  FFTLIB_fft1dBatched_i16sc_c16sc_o16sc_d.c */
/* ======================================================================== */
