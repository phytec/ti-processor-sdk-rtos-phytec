...
{
  int32_t privBufSize = sizeof(FFTLIB_FFT_dftSmall_ixX_cxX_oxX_PrivArgs);
  return privBufSize ;
}
...
FFTLIB_STATUS FFTLIB_FFT_dftSmall_ixX_cxX_oxX_exec_checkParams(
   FFTLIB_kernelHandle handle, 
   ...
   const FFTLIB_bufParams1D_t * bufParamsW)
{
   ...
   if (bufParamsW->data_type == FFTLIB_INT16) {
      status = FFTLIB_FFT_dftSmall_ixX_cxX_oxX_twGen<int16_t>(pKerInitArgs, 
                                                              pW, 
                                                              bufParamsW); 
   } else if (bufParamsW->data_type == FFTLIB_INT32) {
      status = FFTLIB_FFT_dftSmall_ixX_cxX_oxX_twGen<int32_t>(pKerInitArgs, 
                                                              pW, 
                                                              bufParamsW);
   } else {
      status = FFTLIB_ERR_NOT_IMPLEMENTED;
   ...
   FFTLIB_FFT_dftSmall_ixX_cxX_oxX_InitArgs *pKerInitArgs)
{
   ...
   {
      status = FFTLIB_FFT_dftSmall_ixX_cxX_oxX_init_cn(handle, 
                                                       bufParamsX,
                                                       bufParamsW,
                                                       bufParamsY,
                                                       pKerInitArgs) ;
      if (bufParamsX->data_type == FFTLIB_INT16) {
         pKerPrivArgs->execute = 
            FFTLIB_FFT_dftSmall_ixX_cxX_oxX_exec_cn<int16_t, 
                                                    int64_t,
                                                    FFTLIB_MMA_SIZE_16_BIT>;
      } else if (bufParamsX->data_type == FFTLIB_INT32) {
         pKerPrivArgs->execute = 
            FFTLIB_FFT_dftSmall_ixX_cxX_oxX_exec_cn<int32_t, 
                                                    FFTLIB_int128_t,
   ...
   else
   {
      if (bufParamsX->data_type == FFTLIB_INT16) {
         pKerPrivArgs->execute = 
            FFTLIB_FFT_dftSmall_ixX_cxX_oxX_exec_ci<int16_t,
                                                    FFTLIB_MMA_SIZE_16_BIT>;
         status = FFTLIB_FFT_dftSmall_ixX_cxX_oxX_init_ci<FFTLIB_INT16>(handle, 
                                                                        bufParamsX, 
                                                                        bufParamsW, 
                                                                        bufParamsY, 
                                                                        pKerInitArgs);
         ...
         pKerPrivArgs->execute = 
            FFTLIB_FFT_dftSmall_ixX_cxX_oxX_exec_ci<int32_t,
                                                    FFTLIB_MMA_SIZE_32_BIT>;
         status = FFTLIB_FFT_dftSmall_ixX_cxX_oxX_init_ci<FFTLIB_INT32>(handle, 
                                                                        bufParamsX,
                                                                        bufParamsW,
                                                                        bufParamsX,
                                                                        pKerInitArgs);
...
FFTLIB_STATUS FFTLIB_FFT_dftSmall_ixX_cxX_oxX_exec(
   FFTLIB_kernelHandle handle, 
   void *restrict pX,
   void *restrict pW,
   void *restrict pY)
