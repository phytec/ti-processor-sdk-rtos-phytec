...
/* CODE_SECTION (FFTLIB_ifft1d_i32fc_c32fc_o32fc_cn, ".text:ansi") */
/* CODE_SECTION (FFTLIB_ifft1d_i32fc_c32fc_o32fc_checkParams_cn, ".text:ansi")
 */
...
void ifft_i32fc_o32fc_conjugate_cn (void *pX, uint32_t size)
{

   float   *pXLocal = (float *) pX;
   float   *pX_ptr  = pXLocal;
   uint32_t i       = 0;
   for (i = 1; i < (size << 1); i += 2) {
      pXLocal[i] = (-1.0f) * pX_ptr[i];
   }
}
                                    ...
                                    FFTLIB_bufParams1D_t *bufParamsY)
{
   ...
   status = FFTLIB_ifft1d_i32fc_c32fc_o32fc_checkParams_cn (
...
#endif
   {
      ...
      ifft_i32fc_o32fc_conjugate_cn (pX, numPoints);
      ...
      for (k = 0; k < numPoints; k++) {
         ...
         pY[2 * k]     = pX[2 * kBitr] / numPoints;
         pY[2 * k + 1] = (-1) * pX[2 * kBitr + 1] / numPoints;
      }
   }
...
FFTLIB_STATUS
FFTLIB_ifft1d_i32fc_c32fc_o32fc_checkParams_cn (
    FFTLIB_F32           *pX,
    FFTLIB_bufParams1D_t *bufParamsX,
    FFTLIB_F32           *pW,
    FFTLIB_bufParams1D_t *bufParamsW,
