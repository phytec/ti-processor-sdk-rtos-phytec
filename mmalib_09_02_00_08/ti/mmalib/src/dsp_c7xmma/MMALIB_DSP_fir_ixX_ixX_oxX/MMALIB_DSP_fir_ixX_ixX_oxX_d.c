...
int MMALIB_DSP_fir_ixX_ixX_oxX_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   int32_t  tpi;
   ...
   MMALIB_DSP_fir_ixX_ixX_oxX_testParams_t   *prm;
   MMALIB_DSP_fir_ixX_ixX_oxX_testParams_t   currPrm;
   MMALIB_DSP_fir_ixX_ixX_oxX_getTestParams(&prm, &test_cases);

   MMALIB_DSP_fir_ixX_ixX_oxX_InitArgs    kerInitArgs;

   int32_t handleSize = MMALIB_DSP_fir_ixX_ixX_oxX_getHandleSize(&kerInitArgs);
   MMALIB_kernelHandle handle = malloc(handleSize);

   TI_profile_init("MMALIB_DSP_fir_ixX_ixX_oxX");
   ...
   for (tpi=0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++){
         ...
         if (currPrm.dataType == MMALIB_INT32 || currPrm.dataType == MMALIB_FLOAT32) {
         ...
         MMALIB_DSP_fir_ixX_ixX_oxX_getSizes(&kerInitArgs,
               &bufParamsIn,
               &bufParamsFilter,
               &bufParamsOut);
         ...
         if( pIn && pOut && pFilter && pOutCn ) {
            ...
            TI_fillBuffer(CONSTANT,
                  0xAB,
                  pIn, NULL,
                  bufParamsIn.dim_x,   // bufParamsIn.dim_x = dataSize * (batchSize - 1) + 
                                       // ((dataSize - filterSize + 1 + MMA_SIZE * MMA_SIZE - 1) / (MMA_SIZE * MMA_SIZE)) * MMA_SIZE * MMA_SIZE
                                       // + ((filterSize + MMA_SIZE - 1) / MMA_SIZE) * MMA_SIZE
                  1,
                  bufParamsIn.dim_x*MMALIB_sizeof(currPrm.dataType),
                  MMALIB_sizeof(currPrm.dataType),
                  testPatternString);

            if (currPrm.dataType != MMALIB_FLOAT32)
            {
            ...
            if (currPrm.dataType != MMALIB_FLOAT32)
            {
               TI_fillBuffer(prm[tpi].testPattern,
                     0,
                     (void *)(((int8_t *)pFilter)+2*MMA_SIZE*MMALIB_sizeof(currPrm.dataType)),
                     prm[tpi].filter,
                     currPrm.filterSize,
                     1,
            ...
            // skip optimized C computation if skip_ci is turned on
            if (skip_ci == 0)
            {
               // prepare kernel for optimized C computation
               status_init = MMALIB_DSP_fir_ixX_ixX_oxX_init_checkParams(
                     handle,
                     &bufParamsIn,
                     &bufParamsFilter,
                     &bufParamsOut,
               ...
               if (status_init == MMALIB_SUCCESS) {
                  ...
                  status_init = MMALIB_DSP_fir_ixX_ixX_oxX_init(handle,
                        &bufParamsIn,
                        &bufParamsFilter,
                        &bufParamsOut,
                        &kerInitArgs);
                  MMALIB_asm(" MARK 1");
                  TI_profile_stop();
                  status_opt = MMALIB_DSP_fir_ixX_ixX_oxX_exec_checkParams(handle, pIn, pFilter, pOut);
               }
               else
               {
               ...
               // perform computation using optimzed C: results at pOut
               if(status_opt == MMALIB_SUCCESS){
                  TI_profile_start(TI_PROFILE_KERNEL_OPT);
                  MMALIB_asm(" MARK 2");
                  status_opt = MMALIB_DSP_fir_ixX_ixX_oxX_exec(handle,
                        pIn,
                        pFilter,
                        pOut);
                  MMALIB_asm(" MARK 3");
            ...
            MMALIB_DSP_fir_ixX_ixX_oxX_init(handle,
                  &bufParamsIn,
                  &bufParamsFilter,
                  &bufParamsOut,
                  &kerInitArgs);

            // perform computation using natural C: results at pOutCn
            TI_profile_start(TI_PROFILE_KERNEL_CN);
            MMALIB_asm(" MARK 8");
            status_nat = MMALIB_DSP_fir_ixX_ixX_oxX_exec(handle,
                  pIn,
                  pFilter,
                  pOutCn);
            MMALIB_asm(" MARK 9");
		...
		if (skip_ci == 0)
		{
			////// compare optimized C result v. natural C result //////
			if (currPrm.dataType != MMALIB_FLOAT32)
			{
            ...
            if (skip_ci == 0)
            {
               ////// compare reference result v. optimized C result //////
               // fprintf(stderr, "Ref v. OPTC\n");
               ...
               // fprintf(stderr, "Ref v. OPTC result:%d\n", status_ref_vs_opt);
            }
            else
            {
               status_ref_vs_opt = TI_TEST_KERNEL_PASS;
            ...
            // only perform ref v. natural C when in skip_ci mode
            if (skip_ci == 1)
            {
               ////// compare reference result v. natural C result //////
               if( currPrm.staticOut != NULL ) {
                  if (currPrm.dataType != MMALIB_FLOAT32)
                  {
                     status_ref_vs_nat = TI_compare_mem_2D((void *)currPrm.staticOut, (void *)pOutCn,
                           tolerance, 65535,
                           bufParamsOut.dim_x,
                           1,
                           bufParamsOut.dim_x*MMALIB_sizeof(bufParamsOut.data_type),
            ...
            currentTestFail = ((status_nat_vs_opt == TI_TEST_KERNEL_FAIL) ||
                  ...
                  (status_ref_vs_nat == TI_TEST_KERNEL_FAIL) ||
            ...
            // print out the results for debug if fail
            if (currentTestFail == 1)
            {
               ...
               MMALIB_PRINTF("\nInput%s", "\n");
               MMALIB_debugPrintBufParams1D(&bufParamsIn);
               MMALIB_debugPrintMatrix1D(pIn, &bufParamsIn);

               MMALIB_PRINTF("\nFilter%s", "\n");
               MMALIB_debugPrintBufParams1D(&bufParamsFilter);
               MMALIB_debugPrintMatrix1D(pFilter, &bufParamsFilter);

               MMALIB_PRINTF("\nOPTC output%s", "\n");
               MMALIB_debugPrintBufParams1D(&bufParamsOut);
               MMALIB_debugPrintMatrix1D(pOut, &bufParamsOut);

               MMALIB_PRINTF("\nNATC output%s", "\n");
               MMALIB_debugPrintBufParams1D(&bufParamsOut);
               MMALIB_debugPrintMatrix1D(pOutCn, &bufParamsOut);

               if (currPrm.staticOut != NULL) 
               {
                  MMALIB_PRINTF("\nREF output%s", "\n");
                  MMALIB_debugPrintBufParams1D(&bufParamsOut);
                  MMALIB_debugPrintMatrix1D((void *)currPrm.staticOut, &bufParamsOut);
               }
            }
            ...
            MMALIB_DSP_fir_ixX_ixX_oxX_perfEst(handle,
                  &bufParamsIn,
                  &bufParamsFilter,
                  &bufParamsOut,
                  &archCycles,
...
int test_main(uint32_t *pProfile) {
   ...
   if( TI_cache_init()) {
      TI_memError("MMALIB_DSP_fir_ixX_ixX_oxX");
      return 1;
   } else
#else
...
#endif
   {
      return MMALIB_DSP_fir_ixX_ixX_oxX_d(&pProfile[0], 0);
   }
}
...
/* ======================================================================== */
/*  End of file:  MMALIB_DSP_fir_ixX_ixX_oxX_d.c                          */
/* ======================================================================== */
