...
/**
******************************************************************************
*  @file     MMALIB_CNN_pixelShuffle_row_ixX_ixX_oxX_idat.h
*
...
#ifndef MMALIB_CNN_PIXELSHUFFLE_ROW_IXX_IXX_OXX_IDAT_H_
#define MMALIB_CNN_PIXELSHUFFLE_ROW_IXX_IXX_OXX_IDAT_H_
...
typedef struct {
   uint8_t testPattern;      /* 0: constant, 1: sequential, 2: random, 3: static array, 4:
                                file, etc */
   void *  staticIn8Bit;     // static feature map buffer for 8 bit
   void *  staticKernel8Bit; // static kernel buffer for 8 bit
   void *  staticOut8Bit;    // static output buffer for 8 bit
   void *  staticIn16Bit;    // static feature map buffer for 16 bit
   void *  staticKernel16Bit; // static kernel buffer for 16 bit
   void *  staticOut16Bit;    // static output buffer for 16 bit
   int32_t kernelWidth;
   ...
   int32_t upscaleFactor; // Upscale factor
   ...
   int32_t inChOffset;  // size of buffer for input feature map in terms of elements. The
                        // buffer can be circular or linear
   int32_t validColsIn; // number of pixels of input feature map including pad for one
                        // kernel call
   ...
   int32_t numOfOutputChKerBuf; // number of output rows in the coefficient buffer. This
                                // is same as subMChannels
   ...
   int32_t biasB;  // constant Bias value for a given layer, other part of bias is part of
                   // coefficient buffer
   ...
   int32_t circularOffset; // starting offset for circular buffer of input feature ma,
                           // only for testing purpose. default use 0.
   ...
   int32_t validColsPerRowIn;     // total pixels per row including left pad for strided
                                  // convolution
   ...
   int32_t testID;
} MMALIB_CNN_pixelShuffle_row_ixX_ixX_oxX_testParams_t;

void MMALIB_CNN_pixelShuffle_row_ixX_ixX_oxX_getTestParams (
    MMALIB_CNN_pixelShuffle_row_ixX_ixX_oxX_testParams_t **params,
    int32_t *                                   numTests);

#endif /* define _MMALIB_CNN_PIXELSHUFFLE_ROW_IXX_IXX_OXX_IDAT_H_ */

/* ======================================================================== */
/*  End of file:  MMALIB_CNN_pixelShuffle_row_ixX_ixX_oxX_idat.h                */
/* ======================================================================== */
