                                             ...
                                             uint8_t   LevelOfFeedback)
{
   ...
   MMALIB_CNN_deconvolve_row_ixX_ixX_oxX_testParams_t *prm = NULL;

   MMALIB_CNN_deconvolve_row_ixX_ixX_oxX_getTestParams (&prm, &test_cases);
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      if (src0 && src1 && dst) {
         ...
         MMALIB_asm (" MARK 0");
         currTestStatus  = MMALIB_CNN_deconvolve_row_ixX_ixX_oxX_init (
         ...
         for (NCount = 0; NCount < totalN; NCount += subN) {
            ...
            for (MCount = 0; MCount < numOutChannels; MCount += subMChannels) {
               ...
               int8_t *src0_Iter = src0;
                  ...
                  MMALIB_asm (" MARK 2");
                  currTestStatus = MMALIB_CNN_deconvolve_row_ixX_ixX_oxX_exec (
      ...
      if(currTestStatus == MMALIB_SUCCESS)
      {
         ...
         {
            MMALIB_PRINTF("MMALIB_CNN_deconvolve_row_ixX_ixX_oxX_init_checkParams failed!%s", "\n");
         }
         currTestStatus = MMALIB_CNN_deconvolve_row_ixX_ixX_oxX_init(
            handle, &src0_addr, &src1_addr, &dst_addr, &kerInitArgs);
         iterN           = 0;
         ...
         {
            MMALIB_PRINTF("MMALIB_CNN_deconvolve_row_ixX_ixX_oxX_init failed with status = %d!\n", currTestStatus);
         }
