...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    if testType == "STATIC":
        in0 = "staticRefIn1Case" + testId
        in1 = "staticRefIn2Case" + testId
...
def gen_idat_file(testParamsFile, testCases):
    ...
    includeString = '#include "MATHLIB_div_testParams.h"'
    ...
    testParamsFile.write("static MATHLIB_div_testParams_t testParams[] =  ")
    testParamsFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    vec_len = int(length / 2) + (length % 2)

    if dTypeIn == "MATHLIB_FLOAT32":
        in0 = np.random.uniform(low=-100, high=100, size=length).astype("float")
        in1_neg = np.linspace(-100, -1, int(length / 2)).astype("float")
        in1_pos = np.linspace(1, 100, vec_len).astype("float")
        in1 = np.concatenate([in1_neg, in1_pos]).astype("float")

    else:
        in0 = np.random.uniform(low=-100, high=100, size=length).astype("double")
        in1_neg = np.linspace(-100, -1, int(length / 2)).astype("double")
        in1_pos = np.linspace(1, 100, vec_len).astype("double")
        in1 = np.concatenate([in1_neg, in1_pos]).astype("double")

    out = np.divide(in0, in1)
...
testParamsFileName = "../../MATHLIB_div/MATHLIB_div_testParams.cpp"
