...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    testParamsFile.write("{\n")
    testParamsFile.write("%s, // Input vector 0 \n" % ((in0)))
    testParamsFile.write("%s, // Input vector 1 \n" % ((in1)))
...
def gen_idat_file(testParamsFile, testCases):
    ...
    includeString = '#include "MATHLIB_atan2_testParams.h"'
    ...
    testParamsFile.write("static MATHLIB_atan2_testParams_t testParams[] =  ")
    testParamsFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    if dTypeIn == "MATHLIB_FLOAT32":
        # use linspace instead of arange for better end point control
        # in0 = np.arange(-1, 1, 2.0 / length).astype("float")
        # in0 = np.linspace(-10, 10, length).astype("float")
        # in1 = np.linspace(-10, 10, length).astype("float")
        ...
        """ Edge Case for outside of arcosine domain """
        # in0 = np.empty(length).astype("float")
        # in0.fill(-1.5)
        # out = np.empty(length).astype("float")
        # out.fill(0x7fffffff)

    else:
        # use linspace instead of arange for better end point control
        # in0 = np.arange(-1, 1, 2.0 / length).astype("double")
        # in0 = np.linspace(-10, 10, length).astype("double")
        # in1 = np.linspace(-10, 10, length).astype("double")
        ...
        """ Edge Case for outside of arcosine domain """
        # in0 = np.empty(length).astype("double")
        # in0.fill(-1.5)
        # out = np.empty(length).astype("double")
        # out.fill(0x7fffffff)

    out = np.arctan2(in0, in1)
...
testParamsFileName = "../../MATHLIB_atan2/MATHLIB_atan2_testParams.cpp"
