...
def gen_idat_file(testParamsFile, testCases):
    ...
    includeString = '#include "MATHLIB_log2_testParams.h"'
    ...
    testParamsFile.write("static MATHLIB_log2_testParams_t testParams[] =  ")
    testParamsFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    out = np.log2(in0)
    if dTypeIn == "MATHLIB_FLOAT32":
        out[in0 > np.finfo(np.float32).max] = 1024.0
        out[in0 <= 0.0] = 0xFF800000
    else:
        out[out > np.finfo(np.float64).max] = 1024.0
        out[out < np.finfo(np.float64).tiny] = np.finfo(np.float64).min
...
testParamsFileName = "../../MATHLIB_log2/MATHLIB_log2_testParams.cpp"
