...
def gen_idat_file(testParamsFile, testCases):
    ...
    includeString = '#include "MATHLIB_pow_testParams.h"'
    ...
    testParamsFile.write("static MATHLIB_pow_testParams_t testParams[] =  ")
    testParamsFile.write("{\n")
...
def process_test_case(testCase):
    ...
    testCase["minX0"] = float(testCase["minX0"])
    testCase["maxX0"] = float(testCase["maxX0"])
    testCase["minX1"] = float(testCase["minX1"])
    testCase["maxX1"] = float(testCase["maxX1"])
...
def gen_test_case_header_file(testCase):
    ...
    minX0 = testCase["minX0"]
    maxX0 = testCase["maxX0"]
    minX1 = testCase["minX1"]
    maxX1 = testCase["maxX1"]

    if dTypeIn == "MATHLIB_FLOAT32":
        in0 = np.random.uniform(minX0, maxX0, length).astype("float")
        in1 = np.random.uniform(minX1, maxX1, length).astype("float")
    else:
        in0 = np.random.uniform(minX0, maxX0, length).astype("double")
        in1 = np.random.uniform(minX1, maxX1, length).astype("double")

    out = np.power(in0, in1)
...
testParamsFileName = "../../MATHLIB_pow/MATHLIB_pow_testParams.cpp"
