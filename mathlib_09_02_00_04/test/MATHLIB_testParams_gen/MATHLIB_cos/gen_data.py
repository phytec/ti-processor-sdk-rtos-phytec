...
def gen_idat_file(testParamsFile, testCases):
    ...
    includeString = '#include "MATHLIB_cos_testParams.h"'
    ...
    testParamsFile.write("static MATHLIB_cos_testParams_t testParams[] =  ")
    testParamsFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    if dTypeIn == "MATHLIB_FLOAT32":
        # in0 = np.arange(0, 2 * np.pi, 2 * np.pi / length).astype("float")
        # use linspace for better end point control
        in0 = np.linspace(-np.pi / 2, np.pi / 2, length).astype("float")
    else:
        # in0 = np.arange(0, 2 * np.pi, 2 * np.pi / length).astype("double")
        in0 = np.linspace(-np.pi / 2, np.pi / 2, length).astype("double")
...
testParamsFileName = "../../MATHLIB_cos/MATHLIB_cos_testParams.cpp"
