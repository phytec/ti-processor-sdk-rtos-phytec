...
def gen_idat_file(testParamsFile, testCases):
    ...
    includeString = '#include "MATHLIB_tan_testParams.h"'
    ...
    testParamsFile.write("static MATHLIB_tan_testParams_t testParams[] =  ")
    testParamsFile.write("{\n")
...
def gen_test_case_header_file(testCase):
    ...
    if dTypeIn == "MATHLIB_FLOAT32":
        in0 = np.linspace(0, 6 * np.pi, length).astype("float")
    else:
        in0 = np.linspace(0, 6 * np.pi, length).astype("double")

    # pi/2, 3*pi/2 undefined for tangent - adjusting respective inputs
    in0[in0 == np.pi / 2] = np.pi / 2 - 0.1
    in0[in0 == 3 * np.pi / 2] = 3 * np.pi / 2 - 0.2
    in0[in0 == 5 * np.pi / 2] = 5 * np.pi / 2 - 0.1
    in0[in0 == 7 * np.pi / 2] = 7 * np.pi / 2 - 0.1
    in0[in0 == 9 * np.pi / 2] = 9 * np.pi / 2 - 0.2
    in0[in0 == 11 * np.pi / 2] = 11 * np.pi / 2 - 0.1
...
testParamsFileName = "../../MATHLIB_tan/MATHLIB_tan_testParams.cpp"
